...
app.patch('/api/v1/profile/edit', authMiddleware, async (req, res):Promise<any> => {
  try {
    const { username, password } = req.body;
    //@ts-ignore
    const user = req.user;
...
app.post('/api/v1/content/add', authMiddleware, async (req, res):Promise<any> => {
  try {
...
app.get('/api/v1/content/view', authMiddleware, async (req, res) => {
  try {
    //@ts-ignore
...
app.patch('/api/v1/content/update/:id', authMiddleware, async (req, res):Promise<any> => {
  try {
...
app.delete('/api/v1/content/delete/:id', authMiddleware, async (req, res):Promise<any> => {
  try {
    const  contentId  = req.params.id;
...
app.post('/api/v1/saved-posts', authMiddleware, async (req, res) => {
  try {
    ...
    const result = await SavedPosts.updateOne(
      { userId: user._id, contentId: contentId }, // Filter
      { $setOnInsert: { userId: user._id, contentId: contentId } }, // Insert this if not existing
...
app.get('/api/v1/saved-posts',authMiddleware, async (req, res) => {
  try {
   // @ts-ignore
   const user= req.user;
...
app.delete('/api/v1/saved-posts/:contentId',authMiddleware, async (req, res) => {
 try {
...
app.patch('/api/v1/brain/share', authMiddleware, async (req, res):Promise<any> => {
  try {
...
app.get('/api/v1/brain/:shareLink', async (req, res):Promise<any> => {
  try {
    ...
    if (!linkRecord) return res.status(404).json({ error: "Invalid share link" });

    const brain = await Brain.findOne({ userId: linkRecord.userId }).populate([
      { path: "content", select: "-userId" },
      { path: "userId", select: "username" }
    ]);

    if (!brain || brain.share === false) {
      return res.status(403).json({ error: "Brain is private or not found" });
    }
