...
import {
  Plus, Star, Filter, Search, LogOut, Twitter, Youtube, FileText, StickyNote, Loader2, Share2, Lock, Brain
} from 'lucide-react';
...
export function DashboardHeader({
  ...
  return (
    <>
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-3">
              <motion.button
                ...
                className="flex items-center justify-center w-9 h-9 bg-slate-200 dark:bg-slate-700 rounded-full font-bold text-slate-800 dark:text-slate-100 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors disabled:opacity-50"
                title="View Profile"
              >
                {isProfileLoading ? (
            ...
            <div className="flex items-center gap-2">
              {/* Share Brain Button - Dynamic based on public/private state */}
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={handleShareBrain}
                disabled={isSharing}
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-md font-medium transition-colors text-sm disabled:opacity-70 ${
                  isBrainPublic
                    ? 'bg-emerald-600 text-white hover:bg-emerald-700'
                    : 'bg-purple-600 text-white hover:bg-purple-700'
                }`}
                title={isBrainPublic ? 'Make Brain Private' : 'Make Brain Public'}
              >
                {isSharing ? (
              ...
              {/* Load Another Brain Button */}
              <motion.button
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                onClick={() => setShowPasteModal(true)}
                className="flex items-center gap-1.5 bg-indigo-600 text-white px-3 py-1.5 rounded-md font-medium hover:bg-indigo-700 transition-colors text-sm"
              ...
              <motion.button
                ...
                className="p-2 text-slate-600 dark:text-slate-400 hover:text-red-600 dark:hover:text-red-500 rounded-md transition-colors disabled:opacity-50"
              >
      ...
      {/* Paste Brain Link Modal */}
      <PasteBrainLinkModal open={showPasteModal} setOpen={setShowPasteModal} />
    </>
  );
}